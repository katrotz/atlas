...
const errorToResponseBody = (errorMessage: string, responseBody: ErrorResponseBody = {errors: []}): ErrorResponseBody => {
...
export const clientError = (err: Error, res: Response, next: NextFunction) => {
  if (err instanceof HTTPClientError) {
    ...
    res.status(err.statusCode).json(
      errorToResponseBody(err.message)
    );
  } else {
    next(err);
  }
...
export const remoteServerError = (err: Error, res: Response, next: NextFunction) => {
  if (err instanceof StatusCodeError) {
    ...
    try {
      ...
      res.status(err.statusCode).json(
        errorToResponseBody(errorResponse.error)
      );
    } catch (parseError) {
...
export const serverError = (err: Error, res: Response, next: NextFunction) => {
  ...
  if (process.env.NODE_ENV === 'production') {
    res.status(500).json(
      errorToResponseBody('Internal Server Error')
    );
  } else {
    res.status(500).json(
      errorToResponseBody(err.stack || err.message)
    );
  }
};
